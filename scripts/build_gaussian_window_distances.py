...
def main():
    ap = argparse.ArgumentParser(
        description="Write an aggregated joined.phy from concat embeddings NPZ using a Gaussian window taper that highlights central positions."
    )
    ...
    ap.add_argument("--sigma", type=float, default=-1.0,
                    help="Gaussian sigma in residues. If <= 0, defaults to window_size/4.")
    ...
    for i, (s, e) in enumerate(windows):
        width = e - s
        w = gaussian_weights(width, sigma)                # (width,)
        W = np.tensordot(w, emb[:, s:e, :], axes=([0], [1]))  # (N, D) weighted mean
    ...
    print(f"Wrote {len(windows)} Gaussian-window PHYLIP files + joined.phy to {outdir}")
